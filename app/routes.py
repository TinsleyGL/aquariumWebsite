...
from app.forms import LoginForm,RegistrationForm, CreateAquariumForm
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not next_page or url_parse(next_page).netloc != '':
            next_page = url_for('user', username=current_user.username)
        return redirect(next_page)
    return render_template('index.html', title='Sign In', form=form, regForm=regForm)
...
def register():
    ...
    if regForm.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
    return render_template('index.html', form=form, regForm=regForm)
'''
